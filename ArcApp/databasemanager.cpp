...
void DatabaseManager::readPcpThread(QString curClientID, QString type, int idx){
    QString connName = QString::number(dbManager->getDbCounter());
    ...
    bool conn = true;
    {
        ...
        {
            QSqlQuery query(tempDb);
            query.exec("SELECT rowId, Goal, Strategy, Date "
                       "FROM Pcp "
    ...
    emit DatabaseManager::getPcpData(goal, strategy, date, idx);
}


//QSqlQuery DatabaseManager::readPcpThread(QString curClientID, QString type, int idx){
//    QString connName = QString::number(dbManager->getDbCounter());
//    QSqlDatabase tempDb = QSqlDatabase::database();

//    dbManager->createDatabase(&tempDb, connName);

//    QSqlQuery result(tempDb);
//    result.exec("SELECT rowId, Goal, Strategy, Date "
//               "FROM Pcp "
//               "WHERE ClientId = " + curClientID +
//               " AND Type = '" + type + "'");

//    QSqlQuery tmp(result);
//    tempDb.close();
//    QSqlDatabase::removeDatabase(connName);
